...
test("Handling Input Field related scenario in PW_JS", async ({page}) => {
    ...
    await expect(page.locator('h1[class*="title"]')).toHaveText("Input");
    ...
    const exp_getText = await page.locator("#getMe").innerText();
    expect(exp_getText).toEqual("ortonikc");
    const olf_text = await page.locator("#clearMe").toHaveText();
